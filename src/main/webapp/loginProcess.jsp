...
<%@ page import="com.phone.model.User" %>
...
<%
    ...
    // 콘솔 로그 찍기
    System.out.println("로그인 요청: id=" + userId + ", passwd=" + userPasswd);
	/*userid passwd가 비어있을때 로그인 실패*/
...
<%
        ...
        User user = userService.loginUser(userId, userPasswd);
...
%>
            <h2>로그인 성공!</h2>
            <p>환영합니다, <%= user.getNickname() %>님.</p>
            <a href="mainpage.jsp" class="btn btn-primary mt-3">메인 페이지로 이동</a>
<%
        } else {
